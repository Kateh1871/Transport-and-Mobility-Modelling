        ...
        for j, ped_j in pedestrians.items():
            if i!=j:
                ...
                # apply negative (attractive) force if both peds are group 2
                    ...
                    ped_i.acc -= A_social*angular_dependency*math.exp(-(distance)/B_social)*tangent# Forces from boundaries
...
world_definition = {
    "space": {
        ...
        'left': {'type': 'rectangle', 'coordinates': [-1, 0, -2, corridor_width], 'colour': 'green', 'add_boundaries': False},

    },
    "pedestrians": {
        "group1": {"source": "corridor", "destination": "right", "colour": "red", "birth_rate": 99, "max_count": 10},
        'group2': {'source': 'corridor', 'destination': 'left', 'colour': 'green', 'birth_rate': 99, 'max_count': 4}
